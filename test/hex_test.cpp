...
TEST(Hex, ShortString) {
	...
	string decoded = encoder.Decode(str_short_hex);
...
TEST(Hex, LongString) {
	...
	string decoded = encoder.Decode(str_long_hex);
...
TEST(Hex, BadLength) {
	...
	EXPECT_NO_THROW(encoder.Encode((unsigned char *) bad_len.c_str(), bad_len.size()));
	ASSERT_THROW(encoder.Decode(bad_len), std::invalid_argument);
}
